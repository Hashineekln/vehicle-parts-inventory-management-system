...
function Transaction() {
    ...
    const filteredTransactions = transactions.filter(tx => {
        if (searchQuery.trim() === '') {
            ...
            const part = vehicleParts.find(vp => vp.part_no === tx.vehicle_part_part_no);
            const supplier = suppliers.find(sp => sp.supplier_id === tx.supplier_supplier_id);
            const shelf = shelves.find(s => s.shelf_id === tx.shelf_id);
                    ...
                    <table className='w-full text-sm text-left text-gray-900'>
                        ...
                        <tbody>
                            {filteredTransactions.map((tx, index) => (
                                <tr key={tx.transaction_id + '-' + index} className='bg-white border-b'>
